...
ffmpeg -i $1 -c:v libx264 -crf 19 -vf "scale=$2:-2" -fps_mode cfr -movflags faststart -an $3/${FILENAME}_h264.mp4
ffmpeg -i $1 -c:v libvpx-vp9 -crf 29 -deadline best -vf "scale=$2:-2" -fps_mode cfr -an $3/${FILENAME}.webm
ffmpeg -i $1 -c:v libx265 -crf 22 -preset veryslow -vf "scale=$2:-2" -fps_mode cfr -tag:v hvc1 -movflags faststart -an $3/${FILENAME}_h265.mp4
